...
import { useModels, useAddModel, useDeleteModel, useUpdateModels, useResetModels } from '@/hooks/useModels'
import { DndContext, closestCenter, type DragEndEvent, PointerSensor, useSensor, useSensors } from '@dnd-kit/core'
import { SortableContext, useSortable, verticalListSortingStrategy, arrayMove } from '@dnd-kit/sortable'
import { CSS } from '@dnd-kit/utilities'
import { Plus, Trash2, Key, Wifi, WifiOff, GripVertical, RotateCcw, Settings2, X, Check, ChevronsUpDown, ExternalLink } from 'lucide-react'
...
const MODEL_SUGGESTIONS: ModelSuggestion[] = [
  ...
  { label: 'Codex / GPT-5.3-Codex', id: 'codex/gpt-5.3-codex', provider: 'codex' },
  ...
  { label: 'Claude Code / Fable 5', id: 'claude-code/fable-5', provider: 'claude-code' },
  { label: 'Claude Code / Opus 4.8', id: 'claude-code/opus-4-8', provider: 'claude-code' },
  { label: 'Claude Code / Opus 4.7', id: 'claude-code/opus-4-7', provider: 'claude-code' },
  { label: 'Claude Code / Sonnet 4.6', id: 'claude-code/sonnet-4-6', provider: 'claude-code' },
  { label: 'Claude Code / Haiku 4.5', id: 'claude-code/haiku-4-5', provider: 'claude-code' },
  { label: 'Gemini', id: 'gemini', provider: 'gemini' },
  { label: 'Antigravity', id: 'agy', provider: 'agy' },
  { label: 'Antigravity / Gemini 3.5 Flash', id: 'agy/gemini-3-5-flash', provider: 'agy' },
  { label: 'Antigravity / Claude Sonnet 4.6', id: 'agy/claude-sonnet-4-6', provider: 'agy' },
  { label: 'Antigravity / Claude Opus 4.6', id: 'agy/claude-opus-4-6', provider: 'agy' },
  { label: 'Cline', id: 'cline', provider: 'cline' },
  { label: 'Cursor', id: 'cursor', provider: 'cursor' },
  { label: 'Goose', id: 'goose', provider: 'goose' },
  { label: 'Goose / Claude Sonnet 4.6', id: 'goose/anthropic-claude-sonnet-4-6', provider: 'goose' },
...
function SortableModelRow({ model, onDelete, onConfigure }: { model: Model; onDelete: (id: string) => void; onConfigure: (model: Model) => void }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: model.id })
  const style = { transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.5 : 1 }

  return (
    <div ref={setNodeRef} style={style} className="flex items-center gap-2 px-3 py-2 rounded-md border border-border/40 bg-muted/20">
      ...
      <span className={cn(
        ...
        PROVIDER_BADGE_COLORS[model.provider] ?? 'bg-muted text-muted-foreground'
      )}>
        {PROVIDER_LABELS[model.provider] || model.provider}
      </span>
      <code className="text-[10px] text-muted-foreground bg-muted px-1.5 py-0.5 rounded font-mono hidden sm:inline">{model.id}</code>
      <Button variant="ghost" size="icon-sm" className="h-6 w-6 text-muted-foreground hover:text-foreground shrink-0"
...
function InlineModelForm({
  mode, model, onSave, onCancel, onDelete,
}: {
  mode: 'add' | 'edit'
  model?: Model
  ...
  const filteredSuggestions = useMemo(() => {
    ...
    return MODEL_SUGGESTIONS.filter(s => {
      ...
      return s.id.toLowerCase().includes(modelId.toLowerCase()) ||
  ...
  return (
    <div ref={formRef} className="border border-border/60 rounded-lg bg-muted/30 p-3 space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold">{mode === 'add' ? 'Add Model' : 'Configure Model'}</span>
        <Button variant="ghost" size="icon-sm" className="h-5 w-5 text-muted-foreground" onClick={onCancel}>
          <X className="w-3 h-3" />
        </Button>
      </div>
      <div className="grid grid-cols-2 gap-2">
        <div className="space-y-1">
          ...
          <Select value={provider || null} onValueChange={(v) => { setProvider(v ?? ''); setModelId('') }}>
        ...
        <div className="space-y-1">
          <label className="text-[10px] font-medium text-muted-foreground">Model ID</label>
          <Popover open={showSuggestions} onOpenChange={setShowSuggestions}>
            <PopoverTrigger
              render={
                <Button variant="outline" role="combobox" aria-expanded={showSuggestions} className="w-full h-8 justify-between text-xs font-normal">
                  ...
                  <CommandEmpty>
                    {modelId ? (
                      <button
                        ...
                        onClick={() => { setModelId(modelId); setShowSuggestions(false) }}
                      >
      ...
      <div className="space-y-1">
        <label className="text-[10px] font-medium text-muted-foreground">Displayed name</label>
        <div className="h-8 flex items-center px-3 rounded-lg border border-input/50 bg-input/20 text-xs text-muted-foreground">
          {displayLabel || <span className="italic">Select provider and model</span>}
        </div>
      </div>
      <div className="flex items-center justify-between pt-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" className="h-7 text-xs" onClick={handleSave} disabled={!modelId || !provider || !displayLabel}>
...
export function ModelsTab() {
  ...
  const handleAddCred = async () => {
    ...
    await addCredential.mutateAsync({ name: newName, value: newValue, provider: newProvider || undefined })
    setNewName('')
    setNewValue('')
    setNewProvider('')
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => apiFetch('/api/config/open', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ scope: 'global' }) })}
            ...
            {Object.entries(groupedModels).map(([provider, providerModels]) => (
              <div key={provider} className="space-y-1">
                ...
                {providerModels.map(m => (
                  <SortableModelRow key={m.id} model={m}
      ...
      <div className="flex items-center justify-between pt-1">
        <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => resetModels.mutate()}
          disabled={resetModels.isPending}>
          <RotateCcw className="w-3 h-3 mr-1" /> Reset to defaults
        </Button>
        <Button size="sm" className="h-7 text-xs" onClick={handleAddClick} disabled={!!showForm}>
      ...
      <div className="space-y-2">
        ...
        {credentials.map(cred => {
          ...
          const isConnected = cred.provider ? connectedNames.includes(cred.provider) : false

          return (
            <div key={cred.name} className="flex items-center gap-2 px-3 py-2 rounded-md border border-border/60">
              <Key className="w-3.5 h-3.5 text-muted-foreground shrink-0" />
              <span className="text-xs font-mono flex-1">{cred.name}</span>
              {cred.provider && (
                <span className={cn(
                  'text-[10px] px-1.5 py-0.5 rounded-full font-medium',
                  PROVIDER_BADGE_COLORS[cred.provider] ?? 'bg-muted text-muted-foreground'
                )}>
                  {cred.provider}
                </span>
              )}
              {isAcp && isConnected && (
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-secondary text-secondary-foreground font-medium">Connected</span>
              )}
              {isAcp && isConnected && (
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-2">
          <Input placeholder="Name (e.g. ANTHROPIC_API_KEY)" className="flex-1 h-8 text-xs"
            value={newName} onChange={e => setNewName(e.target.value)} />
        </div>
        <div className="flex items-center gap-2">
          <Input placeholder="Value" type="password" className="flex-1 h-8 text-xs"
            value={newValue} onChange={e => setNewValue(e.target.value)} />
          <Select value={newProvider || null} onValueChange={(v) => setNewProvider(v ?? '')}>
          ...
          <Button size="sm" className="h-8 text-xs" onClick={handleAddCred} disabled={!newName || !newValue}>
