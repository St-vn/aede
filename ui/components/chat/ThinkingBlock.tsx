...
export function ThinkingBlock({ thinking, isStreaming, isThinkingActive }: Props) {
  ...
  return (
    <div className="border border-border rounded-lg my-2 overflow-hidden">
      <button
        type="button"
        onClick={() => thinking && setIsOpen(!isOpen)}
        disabled={!thinking}
        className="flex items-center gap-2 w-full px-3 py-2 text-xs text-muted-foreground
                   hover:text-foreground hover:bg-muted/50 transition-colors disabled:cursor-default disabled:hover:bg-transparent disabled:hover:text-muted-foreground"
      >
        <Brain className="w-3.5 h-3.5 shrink-0" />
        <span className="font-medium">Thinking</span>
        {thinking && <span className="text-[10px] text-muted-foreground/60">{charCount} chars</span>}
        {isStreaming && (
          <span className="inline-block w-2 h-2 rounded-full bg-yellow-500 animate-pulse ml-auto" />
        )}
        {thinking && <ChevronDown className={`w-3.5 h-3.5 ml-auto transition-transform ${isOpen ? 'rotate-180' : ''}`} />}
      </button>
      <div className={`grid transition-all duration-200 ease-out ${isOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
        <div className="overflow-hidden min-h-0">
          <div className="px-3 pb-2 max-h-[300px] overflow-y-auto scroll-thin">
