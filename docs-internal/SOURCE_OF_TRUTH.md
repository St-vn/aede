...
├── acp/
│   ├── client.py        # async JSON-RPC message-pump (AcpClient)
│   ├── registry.py      # ACP AgentConfig registry + seed_default_agents
│   ├── manager.py       # AcpManager (async connect/disconnect, switch)
│   ├── session.py       # AcpSession wrapper (async)
│   ├── auth.py          # drive_auth async generator (AuthStep events)
│   ├── permissions.py   # ACP permission→gate bridge
...
The ACP transport was rewritten (`adbb00f`, `8e3dbb3`) from a blocking
`subprocess.Popen` + `ThreadPoolExecutor` design into an **async JSON-RPC
message-pump** modeled on the official ACP TypeScript SDK `Connection` and the
MCP Python SDK stdio transport. This fixed thread-pool exhaustion (stuck turns
leaked unkillable threads), concurrent-turn pipe races, event-loop blocking, and
enabled live streaming + long (15min+) turns with no timeout. Earlier fixes in
the lineage: `815c877` (registry seeding, sub-model resolution, session adoption),
`426bf2e` (session/new timeout), `3383a42` (connect off the event loop — now
subsumed by the full async rewrite).

### ACP Client — async message-pump

**File:** `aede/acp/client.py` (436 lines)

JSON-RPC 2.0 over an **`asyncio.create_subprocess_exec`** stdio subprocess (NOT
`subprocess.Popen`). `_STREAM_LIMIT = 16 MB` is passed as the StreamReader
`limit=` — the asyncio default of 64 KB overflows on real ACP NDJSON lines (file
contents, tool results) and crashes the reader with `LimitOverrunError`.

**Message-pump core:**
- `_read_loop()` — one async reader task over stdout; reads NDJSON line-by-line, `json.loads`, `_dispatch`.
- `_pending: dict[int|str, asyncio.Future]` + `_next_id` — outstanding requests keyed by id.
- `_write_lock: asyncio.Lock` — serializes byte-writes to stdin (concurrent sends never interleave).
- `send_request(method, params)` — allocate id → register future → `_write` → `await future` (**no timeout on `session/prompt`** — long turns are fine).
- `send_notification(method, params)` — write-only, no id.
- `_dispatch(msg)` routes by shape: response (id, no method) → resolve `_pending[_norm_id(id)]`; notification (method, no id) → `_handle_notification`; inbound request (method + id) → `_handle_inbound_request`. `_norm_id` normalizes string↔int ids (agents may echo `"0"`).
- EOF / `aclose()` fail all pending futures (awaiters never hang).

**Streaming + cancel:**
- `_session_sinks: dict[sessionId, callable]`. `prompt(session_id, text, on_update)` registers a per-session sink that accumulates `agent_message_chunk` text and fires `on_update`; returns the result with `text` = concatenated chunks. Routing is by `params.sessionId`, so concurrent sessions don't cross-contaminate.
- `cancel(session_id)` → `session/cancel` notification (the prompt future then resolves with `stopReason=cancelled`).

**Agent→client reentrant requests** (sent DURING a prompt — without handlers the agent stalls the turn with no output):
- `_register_default_handlers()` (called in `__init__`) registers defaults; `set_request_handler(method, handler)` overrides.
- `_default_permission` — auto-approves `session/request_permission` (NOT capability-gated, always required). Picks the option whose `kind` is `allow_once` (fallback `allow_always`, then first) and returns `{"outcome": {"outcome": "selected", "optionId": <that option's id>}}`.
- `_default_fs_read` / `_default_fs_write` — handle `fs/read_text_file` / `fs/write_text_file`.
- `_handle_inbound_request` replies `-32601` for any unhandled method.
- **`clientCapabilities` advertises `fs` read/write but `terminal: False`** — capabilities are a binding contract, so with terminal off the agent runs commands itself rather than sending `terminal/*` requests aede doesn't implement.

**Lifecycle (all async):** `start()`, `initialize()`, `new_session(timeout=60.0)`, `aclose()` (cancels reader/stderr tasks, terminates subprocess). **Sub-model env injection preserved** in `_inject_env`/`_build_cmd`: `ANTHROPIC_MODEL` for claude-code, `GOOSE_PROVIDER`+`GOOSE_MODEL` for goose, `--model` flag for codex, Windows `.cmd`/`.bat` wrapping.

**Dataclasses:** `InitializeResult` (incl. `auth_methods`), `AgentInfo`, `AcpError` (`.code`, `.message`).

### ACP Registry

**File:** `aede/acp/registry.py`  
**Path:** `~/.aede/agents.json`  
**Config:** `AgentConfig(name, transport, command, args, credentials_ref, model_override)`  
`seed_default_agents(registry)` seeds the 7 base agents at startup (only-add-if-missing; never clobbers user `agents.json`) so connect-by-name works without prior credential setup.

### ACP Manager

**File:** `aede/acp/manager.py` (91 lines)

| Method | Async? | Description |
|--------|--------|-------------|
| `connect(name)` | **async** | `async for step in drive_auth(...)`; spawns AcpClient, adopts the session id created during the handshake (`session.adopt`) rather than re-creating it |
| `disconnect(name)` | **async** | `await client.aclose()`, auto-fallback to next connected agent |
| `switch_to(name)` | sync | Activate a different already-connected agent |
| `active_session()` | sync | Currently focused agent |
| `list_connected()` | sync | All connected agent names |

### ACP Session

**File:** `aede/acp/session.py` (59 lines)  
Wrapper: `async create(cwd, mcp_servers)` → session id, `async prompt(text, on_update=…)` → `PromptResult(stop_reason, text, raw)`, `adopt(session_id)` (sync — binds an id created during auth).

### ACP Permissions Bridge

**File:** `aede/acp/permissions.py`  
`AcpPermissionBridge` maps ACP permission outcomes to aede's approval gate. **Note:** the live flow currently auto-approves inside the client (`_default_permission`); this bridge is the seam for routing permissions through aede's gate instead, not the active path.

### ACP Credential Provider

**File:** `aede/acp/credentials.py`  
Injects credentials from the vault into the agent subprocess environment.

### ACP Chat Routing

**File:** `aede/provider.py`

`get_provider(cfg, acp_manager)` intercepts before LLM-provider selection: if `cfg.model` is in `ACP_MODEL_IDS`, returns an `AcpProvider` (raises if no `acp_manager`).

**`ACP_MODEL_IDS`** — frozenset of routable model ids: base agents (`codex`, `claude-code`, `gemini`, `agy`, `cline`, `cursor`, `goose`, `opencode`) plus sub-model entries (e.g. `codex/gpt-5.5`, `claude-code/opus-4-8`, `goose/openai-gpt-4o`).

**`AcpProvider`** — implements `Provider.stream_turn()`. Per turn (all **`await`**, no `asyncio.to_thread`): resolves base agent + sub-model override, `await manager.disconnect`/`connect` (or `switch_to`), builds a prompt string via `_build_prompt_text()`, and `await session.prompt(prompt_text, on_update=…)`. `__init__` takes a `stream_text` async callback; `_make_on_update` routes each `agent_message_chunk` to it via `asyncio.ensure_future(stream_text(text))`. Returns a `NormalizedResponse` (token counts 0 — ACP agents don't report usage).

`acp_manager` is threaded from `cli.py`/`server.py` → `AgentLoop` → `get_provider()`.

### Streaming wiring (`8e3dbb3`)

A single `stream_text` async callback unifies token streaming across all providers:
- `Provider.stream_turn` gained a `stream_text` param; Anthropic/OpenAI send tokens to it (else fall back to `console.print`).
- `AgentLoop` carries `stream_text` and `_get_provider()` attaches it to the provider (`provider._stream_text`).
- `server.py`'s `WebSocketConsole` emits streamed tokens (`end=""`) as `text_delta` events and full lines as `console_message`; the WS handler passes an `async _stream_text` into `AgentLoop`. `ChatView.tsx` consumes the `text_delta` stream.
- On `WebSocketDisconnect` the handler **cancels `turn_task`** (no orphaned turns).

### Sub-model Entries (`fff452c`, `815c877`)

`AgentConfig.model_override` drives per-agent env injection at spawn (see ACP Client). Sub-model names use `agent/model` form (e.g. `claude-code/opus-4-8`); `get_acp_model_override` maps them to the concrete model id. Switching sub-models disconnects + respawns (env is spawn-time). This is how subscription auth (the agent's own logged-in Claude/OpenAI/Gemini) is used inside the harness — no API key needed when a subscription exists.

### Client-Driven Auth

**File:** `aede/acp/auth.py` (120 lines)

`drive_auth(agent_name, client, vault, registry)` is an **async generator** yielding `AuthStep` events (`NeedsBrowser`, `NeedsTerminal`, `NeedsKey`, `Progress`, `Connected`, `Failed`) to a CLI or Web adapter. Picks the least-resistance auth path (existing login → vault key → agent OAuth → terminal relaunch). Working — drives `manager.connect`.
