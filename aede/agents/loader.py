...
def load_agents(
    ...
    and each declared tool exists in all_tool_names. Agents with unknown
    skills or tools are skipped with a warning.
    ...
    for name, ad in list(raw.items()):
        for skill_name in ad.skills:
            if skill_name not in skill_registry:
                import sys
                print(f"[yellow]⚠ Agent {name!r} references unknown skill {skill_name!r}, skipping[/yellow]", file=sys.stderr)
                raw.pop(name, None)
                break
        else:
            if ad.tools is not None:
                for tool_name in ad.tools:
                    if tool_name not in all_tool_names:
                        import sys
                        print(f"[yellow]⚠ Agent {name!r} references unknown tool {tool_name!r}, skipping[/yellow]", file=sys.stderr)
                        raw.pop(name, None)
                        break
