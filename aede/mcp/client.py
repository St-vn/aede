...
class MCPBridge:
    ...
    def __init__(self, servers: dict[str, MCPServerConfig]) -> None:
        ...
        self._transport_cms: dict[str, Any] = {}
    ...
    def shutdown_all(self) -> None:
        ...
        async def _close_all() -> None:
            for name in list(self._session_cms.keys()):
                try:
                    cm = self._session_cms.get(name)
                    if cm is not None:
                        await cm.__aexit__(None, None, None)
                except Exception:
                    pass
            for name in list(self._transport_cms.keys()):
                try:
                    cm = self._transport_cms.get(name)
                    if cm is not None:
                        await cm.__aexit__(None, None, None)
                except Exception:
                    pass
