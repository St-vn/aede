...
def test_skilldefs_from_file_too_large(tmp_path):
    from aede.skills.schema import SkillDef, SkillLoadError
    md_file = tmp_path / "huge.md"
    with md_file.open("w", encoding="utf-8") as f:
        f.write("---\nname: huge\ndescription: too big\n---\n\n")
        f.write("x" * (11 * 1024 * 1024))
    with pytest.raises(SkillLoadError, match="exceeds maximum"):
        SkillDef.from_file(md_file)

def test_skilldefs_from_file_unknown_field_dropped(tmp_path):
    from aede.skills.schema import SkillDef
...
""")
    ...
    assert not hasattr(sd, "unknownField")
